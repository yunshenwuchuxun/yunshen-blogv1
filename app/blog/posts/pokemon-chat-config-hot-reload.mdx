---
title: 'pokemon agent runtime 系列（五）：配置热切换与运行时覆盖'
...
<PaperCard
	title="pokemon agent runtime · Runtime 5"
	description="这篇文章聚焦配置与运行时行为，不讲模型能力本身，而是讲清楚：为什么前端设置页点一下开关，后端行为就会变；以及这种热切换设计到底靠哪些机制支撑。"
	links={[
		{ label: 'Runtime 6：Docker Compose 架构与服务分层', url: '/blog/pokemon-chat-docker-architecture' },
	]}
/>

> 系列导航：上一篇见 **[pokemon agent runtime 系列（四）：LangGraph Agent 编排](/blog/pokemon-chat-agent-orchestration)**；下一篇见 **[pokemon agent runtime 系列（六）：Docker Compose 架构与服务分层](/blog/pokemon-chat-docker-architecture)**。本文负责回答：**为什么页面上的开关，能直接改变后端运行时行为。**
