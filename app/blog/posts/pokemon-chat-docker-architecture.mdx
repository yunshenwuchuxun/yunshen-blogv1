---
title: 'pokemon agent runtime 系列（六）：Docker Compose 架构与服务分层'
...
<PaperCard
	title="pokemon agent runtime · Runtime 6"
	description="这篇文章聚焦部署与基础设施层，解释这个项目为什么不能只被看作‘跑几个容器’，而应该被理解成：如何给一个多能力 AI 系统划分主干服务和能力依赖。"
	links={[
		{ label: 'Runtime 7：LangGraph 工程化实战', url: '/blog/pokemon-chat-langgraph-engineering' },
	]}
/>

> 系列导航：上一篇见 **[pokemon agent runtime 系列（五）：配置热切换与运行时覆盖](/blog/pokemon-chat-config-hot-reload)**；下一篇见 **[pokemon agent runtime 系列（七）：LangGraph 工程化实战](/blog/pokemon-chat-langgraph-engineering)**。本文负责回答：**这些能力在 Docker Compose 里到底是怎样被组织起来的。**
