---
title: 'pokemon agent runtime 系列（四）：LangGraph Agent 编排'
...
<PaperCard
	title="pokemon agent runtime · Runtime 4"
	description="这篇文章用 pokemon agent 作为例子，解释 Agent 模式如何从一个抽象概念，落到可读、可调试、可约束的 LangGraph 执行图上。"
	links={[
		{ label: 'Runtime 5：配置热切换与运行时覆盖', url: '/blog/pokemon-chat-config-hot-reload' },
	]}
/>

> 系列导航：上一篇见 **[pokemon agent runtime 系列（三）：本地直答、缓存与 RAG 主链](/blog/pokemon-chat-rag-mainline)**；下一篇见 **[pokemon agent runtime 系列（五）：配置热切换与运行时覆盖](/blog/pokemon-chat-config-hot-reload)**。本文回答一个问题：**当普通模式不够时，系统如何把执行控制权交给 supervisor、workers 和 finalizer？**
