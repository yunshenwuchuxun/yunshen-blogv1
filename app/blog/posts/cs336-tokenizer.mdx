---
title: 'CS336 第一讲 · 分词器（Tokenizer）：训练流程、粒度权衡与子词算法'
...
训练只发生一次，产物是 vocab 与 merges；此后每次把文本喂给模型，走的都是一条固定的**编码流水线**。它复用了训练时的前两步（正规化、预切分），再套用训练学到的合并 / 切分规则，最后查表得到整数 ID：

![分词器编码流程：原始文本经正规化、预切分、应用规则得到 token 序列，再查表得到 token ID 序列](/static/blog/tokenizer/encoding-flow.svg)

关键在于这条流水线**完全确定且可逆**：同一段文本无论出现在哪里，都会被切成相同的 token、映射为相同的 ID；解码时按同一套 vocab / merges 反向还原即可拼回原文。下面用 DeepSeek 分词器把这条流程跑一遍。
